...
class Game:
    def __init__(self, terminal_reward, step_reward, apple_reward):
        self.terminal_cubes = []
        ...
        self.apple_pos = (ROWS // 2, ROWS // 2)
        
    def get_startpos(self):
      if RND_START_POS and ROOMS == 4:
        rnd = random.randint(1, 4)
        if rnd == 1 or rnd == 3:
          return (ROWS // 4, ROWS // 4 * rnd)
        else:
          return (ROWS // 4 * 3, ROWS // 4 * (rnd - 1))
      elif RND_START_POS:
    ...
    def step(self, action, mouse_click=None):
        ...
        if self.snake.handle_apple_collision():
    ...
    def draw_apple(self):
        ...
        pygame.draw.rect(screen, (255, 255, 0), (i*dis+1,j*dis+1, dis-2, dis-2))
...
class Snake(object):
    ...
    def handle_apple_collision(self):
        if self.head.pos == self.env.apple_pos:
            self.env.apple_pos = (random.randint(1, ROWS-2), random.randint(1, ROWS-2))
            return True
        return False
